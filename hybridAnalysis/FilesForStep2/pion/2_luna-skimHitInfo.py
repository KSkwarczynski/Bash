...
class glbls:
  ...
  #WARNING
  runlist=[int(filename.split("e")[-1].split(".")[0]) for filename in runlist if ext in filename and "pip" in filename] #get numc files and run numbers
  #particlenames=['oa_pg_pip','oa_pg_proton']
  #WARNING
  particlenames=['oa_pg_pip']
